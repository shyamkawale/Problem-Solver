package Patterns.Heap.Merge_K_Sorted.Maximum_Sum_Combination; 
...
public class Maximum_Sum_Combination extends ProblemSolver { 
    ...
    public int[] maxSumCombinations_UsingOptimizedHeap(int[] nums1, int[] nums2, int k) {        
        ...
        visited.add(idxInString(len1-1, len2-1));
        ...
        for(int i=0; i<k; i++){
            ...
            if(0 <= idx1-1 && idx1-1 < len1 && !visited.contains(idxInString(idx1-1, idx2))){
                maxHeap.offer(new Node(nums1[idx1-1]+nums2[idx2], idx1-1, idx2)); 
                visited.add(idx1-1 +"."+ idx2);
            }
            
            if(0 <= idx2-1 && idx2-1 < len2 && !visited.contains(idxInString(idx1, idx2-1))){
