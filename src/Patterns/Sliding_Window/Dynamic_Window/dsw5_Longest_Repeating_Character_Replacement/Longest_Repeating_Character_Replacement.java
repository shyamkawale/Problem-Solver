...
public class Longest_Repeating_Character_Replacement extends ProblemSolver {
        ...
        while (end < s.length()) {
            ...
            if (remFreq <= k) { // Valid Window
                ...
                while (remFreq > k) {
                    ...
                    start++;
                    remFreq = (end - start + 1) - maxFreq; // why are we not updating maxFreq here even when freqMap has been updated ??❓❓
                }
    ...
    public int characterReplacement_Using_Map(String s, int k) {
        ...
        while (end < s.length()) {
            ...
            maxFreq = Math.max(maxFreq, freqMap.get(endCh));
            int remFreq = (end - start + 1) - maxFreq;
            ...
            if (remFreq <= k) {
                ...
                while (remFreq > k) {
                    ...
                    freqMap.computeIfPresent(startCh, (key, val) -> val == 1 ? null : val - 1);
                    start++;
                ...
                freqMap.computeIfPresent(endCh, (key, val) -> val == 1 ? null : val - 1);
            }
        }
